...
dataframe_list = []
name_list = []
path = "C:\\Users\\hanji\\Desktop\\Heavy tailed data in Finance\\output\\geometric_median\\yearly"
for data in os.listdir(path):
    dataframe_list.append(pd.read_csv(path+'\\'+data,encoding='utf8'))
    name_list.append(data[29:])
...
# compare the geo_metric meidan using the monthly data
dataframe_list = []
name_list = []
path = "C:\\Users\\hanji\\Desktop\\Heavy tailed data in Finance\\output\\geometric_median\\monthly"
for data in os.listdir(path):
    dataframe_list.append(pd.read_csv(path+'\\'+data,encoding='utf8'))
    name_list.append(data[25:])


# Comparison between mean estimator and geometric median
fig1, ax1= plt.subplots(5,1,figsize=(8, 26))

for i in range(5):
    Date = [datetime.strptime(str(dataframe_list[i]['Month'][j]),'%Y-%m') for j in range(len(dataframe_list[i]['Month']))]
    ax1[i].plot(Date[1:], dataframe_list[i]['mean'][1:], label = 'mean estimator')
    ax1[i].plot(Date, dataframe_list[i]['g_median'], label = 'geometric median estimator')
    ax1[i].plot(Date, dataframe_list[i]['median'], label = 'median estimator')
...
ax5[0].plot(range(len(mean)),list(np.array(MeanMedians)[np.argsort(difference)]), color = 'blue', label = 'median-of-means', marker = 'o')
ax5[0].set_title('Difference for Mean and Median-of-means Estimators')
ax5[0].set_xlabel('Run number (ordered by median-of-means error)')
ax5[0].set_ylabel('median-of-means')
ax5[0].legend()
ax5[1].plot(range(len(mean)), np.sort(difference))
ax5[1].set_title('Difference for Mean and Median-of-means Estimators')
ax5[1].set_xlabel('Run number (ordered by median-of-means error)')
ax5[1].set_ylabel('median-of-means')
plt.show()
