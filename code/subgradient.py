...
if __name__ == '__main__':
    ...
    median = []
    with open('g_median_convergence.txt','w') as f:
        for i in year:
            ...
            g_median, distance = subgradient_decent(x,y,epsilon, iterations =2000,learning_rate = 0.01)
            mean.append(x)
            median.append(np.median(y, axis = 0))
            geometric_median_list.append(g_median[-1])
            f.write('For year:' + str(i)+'\n\n')
            f.write('Geomatrix median Point interations:\n\n')
            f.write(str(g_median))
            f.write('\n\n')
            f.write('Distance from the starting point\n\n')
    ...
    for i in range(5):
        ...
        df_temp.to_csv('Geometric_median_subgradient_yearly_'+name, index = False)
