...
def mean_median_equally(data,n):
    if n >1:
        if len(data)>n:
...
# read it again
data_12 = pd.read_csv("C:\\Users\\hanji\\Desktop\\Heavy tailed data in Finance\\data\\2^N_log_returns_sample\\2^12_data.csv", header = None)
data_19 = pd.read_csv("C:\\Users\\hanji\\Desktop\\Heavy tailed data in Finance\\data\\2^N_log_returns_sample\\2^19_data.csv", header = None)
data_21 = pd.read_csv("C:\\Users\\hanji\\Desktop\\Heavy tailed data in Finance\\data\\2^N_log_returns_sample\\2^21_data.csv", header  = None)
data_22 = pd.read_csv("C:\\Users\\hanji\\Desktop\\Heavy tailed data in Finance\\data\\2^N_log_returns_sample\\2^22_data.csv", header = None)
#data_25 = pd.read_csv("C:\\Users\\hanji\\Desktop\\Heavy tailed data in Finance\\data\\2^N_log_returns_sample\\2^25_data.csv", header = None)
...
2^19
'''
log_n_k2 = list(np.linspace(0, 0.9, 100))
scale = [int(len(data_19)**i) for i in log_n_k2]
#scale = pd.Series(scale).unique()
#log_n_k2 = [math.log(i,len(data_19)) for i in scale]
global_median = np.median(data_19)
median_error_19 = []
for i in tqdm(scale):
    temp = []
    
    for j in range(100):
        data = np.random.permutation(data_19)
        temp.append(mean_median_equally(data, i))
    median_error_19.append(np.mean(temp)-global_median)
...
2^21
'''
log_n_k3 = list(np.linspace(0, 0.9, 100))
scale = [int(len(data_19)**i) for i in log_n_k3]
#scale = pd.Series(scale).unique()
#log_n_k3 = [math.log(i,len(data_19)) for i in scale]
global_median = np.median(data_21)
median_error_21 = []
for i in tqdm(scale):
    temp = []
    for j in range(100):
        data = np.random.permutation(data_21)
        temp.append(mean_median_equally(data, i))
    median_error_21.append(np.mean(temp)-global_median)
...
2^22
'''
log_n_k4 = list(np.linspace(0, 0.9, 100))
scale = [int(len(data_22)**i) for i in log_n_k4]
#scale = pd.Series(scale).unique()
#log_n_k4 = [math.log(i,len(data_22)) for i in scale]
global_median = np.median(data_22)
median_error_22 = []
for i in tqdm(scale):
    temp = []
    for j in range(100):
        data = np.random.permutation(data_22)
        temp.append(mean_median_equally(data, i))
    median_error_22.append(np.mean(temp)-global_median)
...
ax6.set_xlabel('alpha index for partitions')
